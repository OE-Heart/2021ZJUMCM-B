...
for day in range(9):
    for member in member_list:
        member.cal_fair_ratio()
    member_list = sorted(member_list)

    # caculate amount of member who is in the company already
    active_member_num = len([each.past_day for each in member_list if (each.past_day >= 0 and each.past_day < each.total_day)])
    ...
    def get_alternative(arr, total_num:int):
        cur_driver = 0
        cur_total = 0
        max_fair_ratio = 0.0
        res_list = []
        ...
        for each in arr:
            if(each not in res_list):
                if(each.fair_ratio <= max_fair_ratio + 1):
                    res_list.append(each)
        return res_list
    ...
    def cal_optimize_ratio(arr, total_num:int):
        opt_ratio = 0.3
        # soft condition 1
    ...
    for possible_solution in list(permutations(alternative_list, work_num * 2)):
        #if(day == 3):
            #print([each.driver for each in possible_solution])
        validator = True
    ...
    print("day %d " % day +str(max_ratio))
    [print(i.mid) for i in ans]
    for member in member_list:
        member.past_day += 1
        if(member.mid == ans[0].mid or member.mid == ans[1].mid):
            member.A_times += 1
        elif (len(ans) > 2 and (member.mid == ans[2].mid or member.mid == ans[3].mid)):
            member.B_times += 1
        elif(len(ans) > 4 and(member.mid == ans[4].mid or member.mid == ans[5].mid)):
            member.C_times += 1
        elif(len(ans) > 6 and(member.mid == ans[6].mid or member.mid == ans[7].mid)):
            member.D_times += 1
